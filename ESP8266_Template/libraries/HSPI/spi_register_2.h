...
/*
	JRO : This is something weird, this header comes from the SDK's ILI9341 example
	project and it redefines a lot of constants defined by the SDK's own header,
	"spi_register.h". It lacks some definition but adds ones that aren't present
	in the SDK.

	It may be that the differences come from there being two different chips :
	- ESP8266
	- ESP8266EX

	The constants that are commented-out in this file have already been defined
	by "spi_register.h", I'm only keeping them here to help understand this header

*/

#ifndef SPI_REGISTER_H_2_INCLUDED
...
// whole register not present in official header !
// renamed from SPI_FLASH_CTRL1 to SPI_CTRL1 for consistency
...
#define SPI_CLKCNT_N 0x0000003F     // this is a mask
#define SPI_CLKCNT_N_S 12           // this is a shift amount
...
// the following is SPI_WR_STATUS(i)
...
/*
// official name : SPI_W<n>(i) in spi_register.h
// Based on the code in hspi.c / hspi.h this is the SPI's data registers,
// at least for writing to the SPI. There are 16 registers of 32 bits, which
// means it's possible to setup a 512-bit (64 bytes) data transfer, presumably
// split into 32-bit transfers.
...
#endif // SPI_REGISTER_H_2_INCLUDED
